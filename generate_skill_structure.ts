...
function generateSkillMd(skillName: string, info?: GenerationInfo): string {
  const normalizedName = normalizeSkillName(skillName);
  const generatedAt = info?.generatedAt || new Date().toISOString();

  let generatedSection = "";
  if (info?.sourceUrl || info?.command) {
    generatedSection = `
## Generated by skillize

- Source: ${info?.sourceUrl || "N/A"}
...
export async function generateSkillStructure(
  ...
  if (!(await exists(skillMdPath))) {
    await fs.writeFile(skillMdPath, generateSkillMd(skillName, generationInfo));
